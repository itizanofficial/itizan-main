  ...
  return (
    <div dir="rtl" className="space-y-6 animate-fade-in pb-10 font-sans">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-[1.5rem] border border-gray-100 dark:border-gray-800 p-2 flex flex-col md:flex-row gap-2 shadow-sm">
        <button onClick={() => setActiveTab('personal')} className={`flex-1 py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all outline-none ${activeTab === 'personal' ? 'bg-[#00838F] text-white shadow-md' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}><User size={18} /> الهوية الأساسية</button>
        <button onClick={() => setActiveTab('professional')} className={`flex-1 py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all outline-none ${activeTab === 'professional' ? 'bg-[#00838F] text-white shadow-md' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}><Award size={18} /> السجل الأكاديمي</button>
        <button onClick={() => setActiveTab('schedule')} className={`flex-1 py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all outline-none ${activeTab === 'schedule' ? 'bg-[#00838F] text-white shadow-md' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}><Clock size={18} /> المناوبات والمواعيد</button>
        <button onClick={() => setActiveTab('security')} className={`flex-1 py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all outline-none ${activeTab === 'security' ? 'bg-[#00838F] text-white shadow-md' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}><Shield size={18} /> بروتوكولات الأمان</button>
      </div>
